...
def run(args):
    ...
    distill_loader = load.dataloader(args.dataset, args.distill_batch_size, True, args.workers)
    ...
    ### load the model to distill
    model.load_state_dict(torch.load(args.distill_model))
    ...
    if args.use_global_roast:
        print("GLOBAL ROAST")
        roaster = FakeRoastUtil_v2.ModelRoasterGradScalerPartial(model, True, sparsity, verbose=FakeRoastUtil_v2.NONE, 
                                                module_limit_size=args.module_limit_size, init_std=args.roast_init_std,
                                                scaler_mode=args.roast_scaler_mode, mapper_args=mapper_args, distill_epochs=args.distill_epochs,
                                                distill_step=args.distill_step)
    ...
    optimizer = opt_class(model.parameters(), lr=args.distill_lr, weight_decay=args.weight_decay, **opt_kwargs)
    print('Roast-Distilling for {} epochs'.format(args.distill_epochs))
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=args.distill_lr_drops, gamma=args.distill_lr_drop_rate)
    best_model_dict, distill_result = train_eval_loop(model, loss, optimizer, scheduler, distill_loader, 
                                  test_loader, device, args.distill_epochs, args.verbose, use_roast_scaler=args.use_roast_grad_scaler, distiller=roaster)

    frames = [distill_result.head(1), distill_result.tail(1)]
    distill_result = pd.concat(frames, keys=['Pre', 'Post-Distill'])
    ...
    best_model_dict, post_result = train_eval_loop(model, loss, optimizer, scheduler, train_loader, 
                                  test_loader, device, args.post_epochs, args.verbose, use_roast_scaler=args.use_roast_grad_scaler, distiller=None)

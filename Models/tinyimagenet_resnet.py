...
def _pt_resnet(arch, block, num_block, base_width, num_classes, dense_classifier, pretrained):
    model = ResNetPT(block, num_block, base_width, num_classes, dense_classifier)
    if pretrained:
        pretrained_path = 'Models/pretrained/{}-cifar{}.pt'.format(arch, num_classes)
        pretrained_dict = torch.load(pretrained_path)
        model_dict = model.state_dict()
...
def pt_resnet18(input_shape, num_classes, dense_classifier=False, pretrained=False):
    """ return a ResNet 18 object
    """
    return _pt_resnet('resnet18', BasicBlockPT, [2, 2, 2, 2], 64, num_classes, dense_classifier, pretrained)

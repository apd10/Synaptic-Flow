    ...
    def __init__(self, plan, num_classes, dense_classifier):
        ...
        for segment_index, (filters, num_blocks) in enumerate(plan):
            for block_index in range(num_blocks):
                ...
                blocks.append(PTBlock(current_filters, filters, downsample))
...
def _pt_resnet(arch, plan, num_classes, dense_classifier, pretrained):
    model = PTResNet(plan, num_classes, dense_classifier)
    if pretrained:
        pretrained_path = 'Models/pretrained/{}-lottery.pt'.format(arch)
        pretrained_dict = torch.load(pretrained_path)
        model_dict = model.state_dict()
